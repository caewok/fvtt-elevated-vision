...
BVHNode getNode(in int idx) {
  ...
  uvec4 dat = texelFetch(uBVHSampler, ivec2(0, idx));
  uvec4 bounds = texelFetch(uBVHSampler, ivec2(1, idx));
  return BVHNode(
    vec2(bounds.xy),  // Min
    vec2(bounds.zw),  // Max
    dat[0],      // leftFirst
 ...
 */
uint decodeEdgeTypes(in uint n) {
  switch ( uSourceType ) {
    case SOURCE_LIGHT: return (n & lightMask) >> OFFSET_LIGHT;
    case SOURCE_SIGHT: return (n & sightMask) >> OFFSET_SIGHT;
    case SOURCE_SOUND: return (n & soundMask) >> OFFSET_SOUND;
    case SOURCE_MOVE:  return (n & moveMask)  >> OFFSET_MOVE;
  }
  return 0;
}
...
bool nodeHasBoundsIntersection(in Ray ray, in BVHNode node) {
  ...
  float tmin = max(minVal.x, minVals.y);
  return tmax > 0.l0 && tmax >= tmin && ray.t2 > (tmin * tmin);
}
...
bool planeRayIntersection(in Plane plane, in Ray ray, out float t) {
  ...
  if ( denom.almostEqual(0) ) return false;
  vec3 delta = plane.point - ray.origin;
  t = dot(delta, plane.normal) / denom;
  return true;
}

float nodeHasObjectIntersection(ray, node) {
  // TODO: Can this be done with less texel fetches? Maybe fetch one to test horizontal first?
  uvec4 dat1 = texelFetch(uEdgeSampler, ivec2(1, node.leftFirst));
  uint wallSenseType = decodeEdgeTypes(dat1[0]);
  if ( wallSenseType == WALL_NONE ) return 0.0;

  uvec4 dat0 = texelFetch(uEdgeSampler, ivec2(0, node.leftFirst));

  vec2 a = vec2(dat0.xy);
  vec2 b = vec2(dat0.zw);
  float top = decodeEdgeElevation(dat1[1])
  float bottom = decodeEdgeElevation(dat1[2]);
  vec3 a3d = vec3(a, top);
  ...
  if ( ix.z > a.z || ix.z < b.z ) return 0.0;
...
float hasIntersection(ray) {
  ...
  if ( currNode.isLeaf ) {
    collision += nodeHasObjectIntersection(ray, node);
  ...
  while ( currLevel >= 0 ) {
    ...
    let goDown = nodeHasBoundsIntersection(ray, currNode);
...
vec3 samplePositionLightSphere(in vec3 fragmentPosition, in float seed) {
  totalCollisions = float(TOTAL_COLLISIONS);
...
void main() {
  ...
  for ( int i = 0; i < TOTAL_COLLISIONS; i += 1 ) {
    ...
    collisions += hasIntersection(ray);
  }
