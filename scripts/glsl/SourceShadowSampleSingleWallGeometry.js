...
export class SourceShadowSampleSingleWallGeometry extends PIXI.Geometry {
  ...
  /** @type {PIXI.Point} */
  get edgeMid() { return this.edge.a.add(this.edge.b).multiplyScalar(0.5); }
  ...
  sourceUpdated(changes, { update = true } = {}) {
    ...
    if ( changed2dPosition || changedElevation || changedLightSize ) {
  ...
  edgeUpdated(changes, { update = true } = {}) {
    ...
    const changed = changedPosition || changedElevation;
    if ( changed ) {
      this._updateGeometry();
      if ( update ) this.update();
    }
...
export class PointSourceShadowSampleSingleWallGeometry extends SourceShadowSampleSingleWallGeometry {
  // ----- NOTE: Getters / Setters ----- //
...
export class DirectionalSourceShadowSampleSingleWallGeometry extends SourceShadowSampleSingleWallGeometry {
